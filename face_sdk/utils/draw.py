...
_static_label_colors = [
    ...
    torch.tensor((255, 250, 79), dtype=torch.float32) / 255.0,  # face
    torch.tensor([255, 125, 138], dtype=torch.float32) / 255.0,  # lb
    torch.tensor([213, 32, 29], dtype=torch.float32) / 255.0,  # rb
    torch.tensor([0, 144, 187], dtype=torch.float32) / 255.0,  # le
    torch.tensor([0, 196, 253], dtype=torch.float32) / 255.0,  # re
    torch.tensor([255, 129, 54], dtype=torch.float32) / 255.0,  # nose
    torch.tensor([88, 233, 135], dtype=torch.float32) / 255.0,  # ulip
    torch.tensor([0, 117, 27], dtype=torch.float32) / 255.0,  # llip
    torch.tensor([255, 76, 249], dtype=torch.float32) / 255.0,  # imouth
    torch.tensor((1.0, 0.0, 0.0), dtype=torch.float32),  # hair
    torch.tensor((255, 250, 100), dtype=torch.float32) / 255.0,  # lr
    torch.tensor((255, 250, 100), dtype=torch.float32) / 255.0,  # rr
    torch.tensor((250, 245, 50), dtype=torch.float32) / 255.0,  # neck
    torch.tensor((0.0, 1.0, 0.5), dtype=torch.float32),  # cloth
...
def _blend_labels(image, labels, label_names_dict=None,
    ...
    if label_names_dict is None:
        ...
        for i in range(1, labels.max() + 1):
            if isinstance(label_names_dict, dict) and i not in label_names_dict:
                bg_mask = torch.logical_or(bg_mask, labels == i)
    ...
    if color_offset is not None:
        ...
        for c in colors:
            nc = c.clone()
            if (nc != torch.zeros(3)).any():
                nc += color_offset
            ncolors.append(nc)
        colors = ncolors

    if image is None:
        image = orig_image = torch.zeros(
            [labels.shape[0], labels.shape[1], 3], dtype=torch.float32)
        alpha = 1.0
    else:
        orig_image = image / torch.max(image)
        image = orig_image * (1.0 - default_alpha)
        alpha = default_alpha
    alpha = torch.tensor(alpha, dtype=torch.float32)
    for i in range(1, torch.max(labels) + 1):
        image += alpha.to(image.device) * \
            torch.tile(
            torch.unsqueeze(
                (labels == i).float().to(image.device), -1),
            [1, 1, 3]) * colors[(i) % len(colors)].to(image.device)
    image[image > 1.0] = 1.0
    image[image < 0] = 0.0
    image[bg_mask] = orig_image[bg_mask]
    return image
...
def _draw_hwc(image: torch.Tensor, data: Dict[str, torch.Tensor]):
    ...
    for tag, batch_content in data.items():
        if tag == 'points':
            for content in batch_content:
                ...
                for x, y in content:
                    ...
                    cc = cc[valid]
                    val = val[valid]
                    val = torch.tensor(val[:, None][:, [0, 0, 0]], dtype=torch.float32)
                    image[rr, cc] = image[rr, cc] * (1.0-val) + val * 255
        ...
        if tag == 'seg':
            ...
            for seg_logits in batch_content['logits']:
                # content: nclasses x h x w
                seg_probs = torch.nn.functional.softmax(seg_logits, dim=0)
                seg_labels = seg_probs.argmax(dim=0).cpu()
                image = (_blend_labels(image.float() /
                         255, seg_labels,
                         label_names_dict=label_names) * 255).to(dtype)

    return image

def draw_bchw(images, data, face_nums):
    """
    Args:
        images: Tensor of shape (B, C, H, W)
        data: List of dictionaries containing data for each image in the batch

    Returns:
        Tensor of shape (B, C, H, W) with drawn images
    """
    batch_size = images.shape[0]
    drawn_images = []

    for i in range(batch_size):
        image = images[i].permute(1, 2, 0)  # Convert to HWC
        start = sum(face_nums[:i])
        end = start + face_nums[i]
        single_data = {"seg": 
                            {
                                "logits": data["seg"]["logits"][start:end],
                                "label_names": data["seg"]["label_names"]
                            }
                        }
        drawn_image = _draw_hwc(image, single_data)
        drawn_images.append(drawn_image.permute(2, 0, 1))  # Convert back to CHW

    return torch.stack(drawn_images)
